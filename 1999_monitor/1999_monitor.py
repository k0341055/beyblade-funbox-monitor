...
# ── 完全不通知的商品關鍵字 ──────────────────────────────
# 商品名稱含以下任一關鍵字 → 完全忽略（不通知、不自動下單）
NOT_NOTIFY_KEYWORDS: list[str] = [
    "BX-43",
]

# ── 略過通知的商品關鍵字 ──────────────────────────────
# 商品名稱含以下任一關鍵字 → 仍會通知，但跳過自動下單
SKIP_KEYWORDS: list[str] = [
    # BX 系列
    "BX-11", "BX-25", "BX-33", "BX-26",
    # BXG 系列
    "BXG-29", "BXG-33",
    # Marvel 聯名
    "蜘蛛人", "鋼鐵人", "薩諾斯", "綠惡魔",
...
BUY_KEYWORDS: list[str] = [
    ...
    "ドラシエルシールド",   # BX-00 ブースター ドラシエルシールド7-60D
    "BX-42", "BX-29", "BX-30",
]
...
async def check_once(page, context=None) -> bool:
    try:
        ...
        # NOT_NOTIFY_KEYWORDS 商品：完全忽略（不通知、不自動下單）
        if NOT_NOTIFY_KEYWORDS:
            ignored_products = [
                p for p in products
                if any(kw.upper() in p["title"].upper() for kw in NOT_NOTIFY_KEYWORDS)
            ]

            if ignored_products:
                log.info(
                    "NOT_NOTIFY_KEYWORDS 商品（完全忽略）："
                    + "、".join(p["title"] for p in ignored_products)
                )

                ignored_hrefs = {p["href"] for p in ignored_products}
                products = [p for p in products if p["href"] not in ignored_hrefs]

        # 如果架上只有不通知商品（例如 BX-43），本輪直接結束
        if not products:
            log.info("架上商品皆屬 NOT_NOTIFY_KEYWORDS，本輪不通知、不下單")
            return False
