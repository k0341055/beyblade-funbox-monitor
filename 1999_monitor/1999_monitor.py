...
BUY_KEYWORDS: list[str] = [
    ...
    "UX-17",
    "ドランソード3-60F",
    "CX-00 エヴァンゲリオンデッキセット",
    "UX-03",
    "CX-00 スターター ティガレイジFT3-60T",
...
async def fetch_products(page) -> list:
    ...
    for card in cards:
        ...
        ).strip() if title_el else "(タイトル不明)"

        maker_el = await card.query_selector(
            "div.c-card__maker"
        )

        release = (
            await maker_el.inner_text()
        ).strip() if maker_el else ""
        ...
        if price_el:
            span = await price_el.query_selector(
                "span"
            )
        ...
        discount = (
            f"{(await discount_el.inner_text()).strip()}%OFF"
            if discount_el
            else ""
        )
...
def _send_order_result_email(
    ...
    """
    個人通知：
    下單結果只寄給 ACCOUNT_1999。
    """

    if (
        not ACCOUNT_1999
        or ACCOUNT_1999 not in GMAIL_RECIPIENTS
    ):
        log.warning(
            "ACCOUNT_1999 不在 GMAIL_RECIPIENTS 中，"
            "無法寄送個人下單結果通知"
        )
    ...
    for href, status in checkout_results.items():
        ...
        label = _CHECKOUT_LABEL_1999.get(
            status,
            status
        )
...
async def _ensure_1999_login(page) -> bool:
    """
    延遲登入：
    ...
    if needs_login:
        if (
            ...
            log.warning(
                "[Amazon Pay] "
                "需要登入但未設定 "
                "AMAZON_ACCOUNT / AMAZON_PASSWORD"
            )
...
async def _checkout_amazon_pay(
    ...
    try:
        ...
        try:
            await page.wait_for_function(
                f"""() => {{
                    ...
                    return (
                        u.includes('{BASE_URL}/orderamazon')
                        ||
        ...
        if (
            "amazon.co.jp" in page.url
            and "/orderamazon" not in page.url
        ):
            auth_result = await _handle_amazon_auth(
                page
            )
...
async def check_once(
    ...
    try:
        ...
        if NOT_NOTIFY_KEYWORDS:
            ...
            if ignored_products:
                ...
                products = [
                    p
                    for p in products
                    if p["href"] not in ignored_hrefs
                ]

        if not products:
            log.info(
                "架上商品皆屬 NOT_NOTIFY_KEYWORDS，"
                "本輪不通知、不下單"
            )
        ...
        notified = {
            h: t
            for h, t in notified.items()
            if h in current_hrefs
        }
                ...
                if not to_checkout:
                    ...
                    if _login_fail_count >= _LOGIN_FAIL_MAX:
                        ...
                        if cart_added:
                            ...
                            status = await _checkout_amazon_pay(
                                page,
                                cart_added
                            )
...
async def main():
    ...
    # 載入 Amazon Pay storage state
    # 注意：
    # 這只是把 Cookie 載入瀏覽器，
    # 不會進入 1999 /mypage，也不會執行登入。
    ...
    if (
        ...
        log.info(
            "載入 Amazon Pay session："
            f"{STORAGE_STATE_FILE}"
        )

    elif AUTO_CHECKOUT:
        log.warning(
            f"找不到 {STORAGE_STATE_FILE}，"
            "Amazon Pay 可能無法自動完成授權"
        )
    ...
    async with async_playwright() as p:
        ...
        # ═════════════════════════════════════
        # 不再於每次 Run 啟動時登入 1999
        #
        # 只有 check_once() 確認真的有
        # BUY_KEYWORDS 商品需要下單時
        # 才會呼叫 _ensure_1999_login()
        # ═════════════════════════════════════
