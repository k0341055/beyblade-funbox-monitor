...
自動偵測兩個玩具電商的新商品，Funbox 偵測到有庫存商品時同時自動登入下單，並透過 Gmail 發送通知。每 5 分鐘由 cron-job.org 觸發一次 GitHub Actions。
...
└────────────────────┬────────────────────┬───────────────────┘
         ...
         └───────────┬──────┘  └──────────┬────────┘
                     │                    │
          ┌──────────▼──────┐  ┌──────────▼────────┐
          │  Playwright +   │  │  requests (登入/  │
          │  Chromium       │  │  加購) +           │
          │  12 輪 / 次      │  │  Playwright (結帳)│
          └──────────┬──────┘  └──────────┬────────┘
                     │                    │
         ┌───────────▼──────┐  ┌──────────▼────────┐
         │  1999.co.jp      │  │ shop.funbox.com.tw│
         │  Beyblade X 頁   │  │  戰鬥陀螺集合頁    │
         │  (有 Cloudflare) │  │  Cyberbiz SPA     │
         └───────────┬──────┘  └──────────┬────────┘
                     │                    │
                     └─────────┬──────────┘
                               │
...
| | Beyblade 監控 | Funbox 監控 |
|---|---|---|
| 目標網站 | `1999.co.jp` | `shop.funbox.com.tw` |
| 偵測商品 | Beyblade X 系列 | 戰鬥陀螺 |
| 反爬蟲 | Cloudflare（需反偵測） | Cyberbiz `/products.json` API |
| 技術架構 | Playwright 全程 | requests 登入/加購 + Playwright 結帳 |
| 每次執行輪數 | 12 輪 | 50 輪（間隔 3~5 秒） |
| 自動下單 | 無 | 有（多帳號平行） |
...
- uses: actions/cache@v4
  with:
    ...
    restore-keys: funbox-seen-   # 永遠取最新的快照
```
